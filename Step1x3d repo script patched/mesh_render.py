...
class MeshRender:
    ...
    def raster_texture(
        ...
        if self.raster_mode == "cr":
            raise ValueError(f"Texture is not implemented in cr")
    ...
    def color_rgb_to_srgb(self, image):
        ...
        if isinstance(image, Image.Image):
            image_srgb = Image.fromarray(
                (image_srgb.cpu().numpy() * 255).astype(np.uint8)
            )
        elif isinstance(image, np.ndarray):
    ...
    def render(
        ...
        image = self._render(
            ...
            self.max_mip_level,
            True,
            filter_mode if filter_mode else self.filter_mode,
        )
    ...
    def render_normal(
        ...
        pos_camera, pos_clip = self.get_pos_from_mvp(
        ...
        visible_mask = torch.clamp(rast_out[..., -1:], 0, 1)
        normal = normal * visible_mask + torch.tensor(
    ...
    def render_position(
        ...
        position = position * visible_mask + torch.tensor(
            bg_color, dtype=torch.float32, device=self.device
        ) * (1 - visible_mask)
        if self.use_antialias:
    ...
    def render_sketch_from_geometry(self, normal_image, depth_image):
        ...
        sketch_image = (
            torch.from_numpy(combined_edges).to(normal_image.device).float() / 255.0
        )
    ...
    def render_sketch_from_depth(self, depth_image):
        ...
        sketch_image = (
            torch.from_numpy(combined_edges).to(depth_image.device).float() / 255.0
        )
        sketch_image = sketch_image.unsqueeze(-1)
        return sketch_image
        ...
        for i in range(len(colors)):
            if isinstance(colors[i], Image.Image):
                colors[i] = torch.tensor(
                    np.array(colors[i]) / 255.0, device=self.device
                ).float()
        if weights is None:
