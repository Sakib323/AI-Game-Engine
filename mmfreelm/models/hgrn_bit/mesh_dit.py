...
# ADDED FOR STABILITY: Zero-init for adaLN emb and final_layer to prevent gradient explosion.
# ADDED FOR STABILITY: Increased norm eps to 1e-5; enabled RoPE by default.
# ADDED: Input norm for x_embedder output.
...
class AdaLayerNormZero(nn.Module):
    ...
    def __init__(self, embedding_dim: int):
        ...
        self.norm = nn.LayerNorm(embedding_dim, elementwise_affine=False, eps=1e-5)  # Increased eps for stability
...
class FluxBitBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, use_rope=True, use_ternary_rope=False, **block_kwargs):  # Enabled RoPE by default
        super().__init__()
        ...
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-5)  # Increased eps
        self.ff = HGRNBitMLP(hidden_size=hidden_size, mlp_ratio=mlp_ratio)
        self.norm2_context = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-5)  # Increased eps
...
class FinalLayer(nn.Module):
    def __init__(self, hidden_size, output_dim):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-5)  # Increased eps
...
class MeshDiT(nn.Module):
    def __init__(
        ...
        use_rope: bool = True,  # Enabled by default for sequence stability
        ...
        self.input_norm = nn.LayerNorm(hidden_size, eps=1e-5)  # ADDED: Normalize embedded inputs for stability
    ...
    def initialize_weights(self):
        ...
        # ADDED FOR STABILITY: Zero-init modulations and final layer (prevents early explosion, like in mesh_dit.py)
        for m in self.modules():
            if isinstance(m, AdaLayerNormZero) or isinstance(m, AdaLayerNormZeroSingle):
                nn.init.constant_(m.emb[1].weight, 0)  # Zero the Linear weights in emb
                nn.init.constant_(m.emb[1].bias, 0)    # Zero the Linear biases in emb
    ...
    def forward(self, x, t, encoder_hidden_states):
        ...
        x_tokens = self.input_norm(self.x_embedder(x))  # ADDED: Norm after embedding
